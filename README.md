...
このrepositoryは、週次価格データを使ってETFの収益とリスクを比較し、その前提条件を確認する研究用プロジェクトです。リターン、volatility、Sharpe ratio、JPX公式snapshotなどの技術語は、比較条件を理解した後で扱います。

READMEの入口は [`KAFKA2306/articles#34`](https://github.com/KAFKA2306/articles/issues/34) の「広い問題 → 具体例 → 技術」の編集原則を維持し、過去snapshotを現在値へ読み替えず、配当調整・通貨・為替ヘッジ等の比較条件を明示します。
